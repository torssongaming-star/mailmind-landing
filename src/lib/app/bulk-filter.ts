 ...
 *
 * Four signal layers (all checked, first match wins, highest confidence first):
 *   0. RFC headers        — List-Unsubscribe / Precedence / Auto-Submitted —
 *                           legally REQUIRED on commercial bulk (CAN-SPAM/GDPR)
 *                           so this is a near-100% confidence signal
...
/**
 * Subset of email headers we care about for bulk detection.
 * Populated by Gmail/Outlook parsers and passed through autoTriage.
 */
export type BulkHeaders = {
  /** RFC 2369 — required on bulk per GDPR/CAN-SPAM. Absolute bulk marker. */
  listUnsubscribe?: string | null;
  /** "bulk" or "list" — explicit RFC indicator of mass mail. */
  precedence?: string | null;
  /** RFC 3834 — auto-generated/auto-replied/auto-notified. */
  autoSubmitted?: string | null;
  /** Set by Microsoft when content was auto-generated by a system. */
  xAutoResponseSuppress?: string | null;
  /** True if ANY X-Campaign-* / X-Mailchimp-* / X-Klaviyo-* / X-Sg-* header was present. */
...
export type BulkSignal =
  | { detected: false }
  | { detected: true; reason: string; layer: "headers" | "sender" | "subject" | "body" };

/**
 * Analyse an incoming email and return whether it looks like bulk/marketing.
 *
 * Layers evaluated in order of confidence (highest first). Function
...
export function detectBulkEmail(params: {
  ...
  subject:   string;
  bodyText:  string;
  /** Optional — when provided enables Layer 0 (highest-confidence header check). */
  headers?:  BulkHeaders;
  /** Optional org-side overrides; when present, filtering can be disabled
   *  entirely OR specific sender domains can be whitelisted. */
  settings?: {
    enabled?:   boolean;
    whitelist?: string[];   // e.g. ["@yoursupplier.se", "important-vendor.com"]
  };
}): BulkSignal {
  // Org has disabled the filter entirely
  if (params.settings?.enabled === false) return { detected: false };

  const from    = params.fromEmail.toLowerCase().trim();
  const subject = params.subject.toLowerCase();
  const body    = params.bodyText.toLowerCase();

  // Whitelist check — never filter mail from these senders
  const whitelist = params.settings?.whitelist ?? [];
  if (whitelist.length > 0) {
    const senderDomain = "@" + from.split("@")[1];
    const isWhitelisted = whitelist.some(entry => {
      const e = entry.toLowerCase().trim();
      if (!e) return false;
      return e === from || e === senderDomain || (e.startsWith("@") && senderDomain === e);
    });
    if (isWhitelisted) return { detected: false };
  }

  // ── Layer 0: RFC headers (highest confidence) ─────────────────────────────
  const h = params.headers;
  if (h) {
    if (h.listUnsubscribe) {
      return { detected: true, layer: "headers", reason: `List-Unsubscribe present` };
    }
    if (h.precedence) {
      const p = h.precedence.toLowerCase().trim();
      if (p === "bulk" || p === "list" || p === "junk") {
        return { detected: true, layer: "headers", reason: `Precedence: ${p}` };
      }
    }
    if (h.autoSubmitted) {
      const a = h.autoSubmitted.toLowerCase().trim();
      // "no" = human-sent. "auto-generated"/"auto-replied"/"auto-notified" = bulk
      if (a && a !== "no") {
        return { detected: true, layer: "headers", reason: `Auto-Submitted: ${a}` };
      }
    }
    if (h.hasESPHeader) {
      return { detected: true, layer: "headers", reason: "ESP campaign header present" };
    }
  }
