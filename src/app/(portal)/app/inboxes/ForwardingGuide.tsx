...
export function ForwardingGuide({ mailmindAddress }: { mailmindAddress: string }) {
  ...
  const CLIENTS: { id: Client; label: string }[] = [
    { id: "outlook-web",  label: t("portal.inboxes.guide.clients.outlookWeb.label") },
    { id: "outlook-app",  label: t("portal.inboxes.guide.clients.outlookApp.label") },
    { id: "gmail",        label: t("portal.inboxes.guide.clients.gmail.label") },
    { id: "apple-mail",   label: t("portal.inboxes.guide.clients.appleMail.label") },
  ];
    ...
    <div className="rounded-2xl border border-cyan-500/20 bg-cyan-500/[0.03] overflow-hidden">
      ...
      >
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="text-sm font-semibold text-white">{t("portal.inboxes.guide.title")}</p>
            <p className="text-xs text-muted-foreground mt-0.5">
              {t("portal.inboxes.guide.description")}
            </p>
          </div>
        </div>
      ...
      {open && (
        <div className="border-t border-cyan-500/10 px-5 pb-5 space-y-4">
          ...
          <div className="rounded-lg bg-black/30 px-3 py-2.5 flex items-center justify-between gap-3 mt-4">
            <div className="min-w-0 flex-1">
              <p className="text-[10px] uppercase tracking-wider text-muted-foreground mb-0.5">
                {t("portal.inboxes.guide.yourAddress")}
              </p>
            ...
            >
              {copied ? t("portal.inboxes.editor.copied") : t("portal.inboxes.editor.copy")}
            </button>
          </div>
          ...
          <div className="rounded-lg border border-white/8 bg-white/[0.02] px-4 py-3 flex items-center gap-3">
            ...
            <div>
              <p className="text-xs font-semibold text-white/70">{t("portal.inboxes.guide.videoComingSoon")}</p>
              <p className="text-[10px] text-muted-foreground mt-0.5">
                {t("portal.inboxes.guide.videoComingSoonDesc")}
              </p>
            </div>
          </div>
