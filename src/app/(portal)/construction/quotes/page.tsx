/**
 * /construction/quotes — list all construction quotes for the org.
 * Reuses the shared quoting-common quote/customer data layer.
 */
...
export default async function ConstructionQuotesPage() {
  ...
  const [quotes, customers] = await Promise.all([
    listQuotes(account.organization!.id, { vertical: "construction" }),
        ...
        {quotes.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-20 text-center">
            ...
            <p className="text-xs text-white/40 mt-1">
              Skapa din första byggoffert via API:et.
            </p>
          </div>
        ) : (
                ...
                {quotes.map((q, i) => (
                  <tr key={q.id} className={i % 2 === 0 ? "bg-white/[0.015]" : ""}>
                    ...
                    <td className="px-4 py-3 text-right text-white/70 tabular-nums">
                      {q.total ? `${Number(q.total).toLocaleString("sv-SE")} kr` : "—"}
                    </td>
